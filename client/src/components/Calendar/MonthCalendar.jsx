...
import { getEvents } from "@/features/eventModal/eventModalSlice";
...
function MonthCalendar() {
  ...
  // Opens the event modal to add a new event
      ...
      <div className="grid grid-cols-7 flex-1 border border-gray-300">
        {days.map((dayObj, index) => {
          ...
          return (
            ...
            >
              <span
                className={`w-8 h-8 flex items-center justify-center rounded-full ${isToday(dayObj) ? "bg-blue-500 text-white font-bold" : ""
                  }`}
              >
                {format(dayObj, "d")}
              </span>
